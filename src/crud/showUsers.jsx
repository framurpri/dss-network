    ...
    <div style={container}>
        ...
        <div style={tableContainer}>
          <table style={table}>
            <thead>
              <tr>
                <th style={{...tableHeader, width:'400px'}}>Usuario</th>
            ...
            <tbody>
              {rowRefs && currentRows.map((row, index) => (
                <tr key={index}>
                  {index % 2 === 0 ? (
                    <td style={{...tableCell, width: '300px'}} ref={rowRefs.current[index]}>
                      <div style={{display: 'flex', alignItems:'center', justifyContent: 'space-between'}}>
                        {row.username}
                        <DropdownMenu onChange={handleRefreshList} user={row} ref={rowRefs.current[index]}/>
                      </div>
                    </td>
                    ) : (
                  ...
                  <td style={tableCell}>{row.last_name ? row.last_name : 'No tiene'}</td>
                  <td style={{...tableCell, backgroundColor:' #eceeee '}}>{row.email ? row.email : 'No tiene'}</td>
                  <td style={tableCell}>{row.phone ? row.phone : 'No tiene'}</td>
